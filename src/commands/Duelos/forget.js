...
module.exports = new Command({
    ...
    execute: async (message, props) => {
        ...
        if (movement.error) return message.reply(createEmbed({
            color: 'red',
            description: movement.error,
        }))
        ...
        if (!movements.includes(movement.name)) return message.reply(createEmbed({
            color: 'yellow',
            description: `${pokemon.pokemon.name} no había aprendido ${movement.name}.`
        }))
        ...
        return message.reply(createEmbed({
            description: `**${pokemon.traits.nickname || pokemon.pokemon.name}** ha olvidado **${movement.name}**.`,
        }))
    }
})